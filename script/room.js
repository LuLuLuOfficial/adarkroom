...
var Room = {
  ...
  init: function (options) {
    ...
    $SM.set(
      "game.fire",
      $SM.get("game.fire.value") === undefined
        ? this.FireEnum.Dead
        : $SM.get("game.fire")
    );
     ...
     */
    if (
      $SM.get("game.builder.level") >= 0 &&
      $SM.get("game.builder.level") < 3
    ) {
      Room._builderTimer = Engine.setTimeout(
        Room.updateBuilderState,
        Room._BUILDER_STATE_DELAY
      );
    }
    if (
      $SM.get("game.builder.level") == 1 &&
    ...
    Notifications.notify(
      ...
      _(
        "the room is {0}",
        Room.TempEnum.fromInt($SM.get("game.temperature.value")).text
      )
    );
    Notifications.notify(
      Room,
  ...
  setTitle: function () {
    var title =
      $SM.get("game.fire.value") < 2 ? _("A Dark Room") : _("A Firelit Room");
  ...
  updateButton: function () {
    ...
    if (
      $SM.get("game.fire.value") == Room.FireEnum.Dead.value &&
      stoke.css("display") != "none"
    ) {
      stoke.hide();
      light.show();
  ...
  stokeFire: function () {
    ...
    if ($SM.get("game.fire.value") < 4) {
      $SM.set(
        "game.fire",
        Room.FireEnum.fromInt($SM.get("game.fire.value") + 1)
      );
    }
  ...
  adjustTemp: function () {
    ...
    if (
      ...
      $SM.set(
        "game.temperature",
        Room.TempEnum.fromInt($SM.get("game.temperature.value") - 1)
      );
      Notifications.notify(
        Room,
        _(
  ...
  updateBuilderState: function () {
    ...
    if (lBuilder === 0) {
      ...
      lBuilder < 3 &&
      $SM.get("game.temperature.value") >= Room.TempEnum.Warm.value
    ) {
      var msg = "";
      switch (lBuilder) {
        case 1:
  ...
  updateStoresView: function () {
    ...
    for (var k in $SM.get("stores")) {
      ...
      if (row.length === 0) {
        ...
        $("div#" + row.attr("id") + " > div.row_val", location).text(
          Math.floor(num)
        );
      }
    }
      ...
      for (var incomeSource in $SM.get("income")) {
        ...
        for (var store in income.stores) {
          if (store == storeName && income.stores[store] !== 0) {
            ...
            if (
              !totalIncome[store] ||
              totalIncome[store].income === undefined
            ) {
  ...
  buyUnlocked: function (thing) {
    if (Room.buttons[thing]) {
      ...
      if (
        thing == "compass" ||
        typeof $SM.get('stores["' + thing + '"]') != "undefined"
      ) {
    ...
    for (var k in Room.Craftables) {
      ...
      if (craftable.button == null) {
        if (Room.craftUnlocked(k)) {
          var loc = Room.needsWorkshop(craftable.type)
            ? craftSection
            : buildSection;
      ...
      if (good.button == null) {
        if (Room.buyUnlocked(g)) {
          good.button = new Button.Button({
            ...
            width: "80px",
            ttPos:
              buySection.children().length > 10 ? "top right" : "bottom right",
          })
            .css("opacity", 0)
            .attr("buildThing", g)
            .appendTo(buySection)
  ...
  compassTooltip: function (direction) {
    var ttPos =
      $("div#resources").children().length > 10 ? "top right" : "bottom right";
    var tt = $("<div>").addClass("tooltip " + ttPos);
    $("<div>")
      .addClass("row_key")
