var Path = {
    ...
    updatePerks: function (ignoreStores) {
        if ($SM.get("character.perks")) {
            ...
            if (!ignoreStores && Engine.activeModule === Path) {
                $("#storesContainer").css({
                    top: perks.height() + 26 + Path._STORES_OFFSET + "px",
                });
            }
        }
    },
    ...
    updateOutfitting: function () {
        ...
        if ($SM.get('stores["kinetic armour"]', true) > 0)
            armour = _("kinetic");
        else if ($SM.get('stores["s armour"]', true) > 0) armour = _("steel");
        ...
        var carryable = $.extend(
            {
                "cured meat": {
                    type: "tool",
                    desc: _("restores") + " " + World.MEAT_HEAL + " " + _("hp"),
                },
                ...
                "medicine": {
                    type: "tool",
                    desc: _("restores") + " " + World.MEDS_HEAL + " " + _("hp"),
                },
            },
            Room.Craftables,
        ...
        for (var k in carryable) {
            ...
            if ((store.type == "tool" || store.type == "weapon") && have > 0) {
                ...
                if (row.length === 0) {
                    ...
                    $(
                        "div#" + row.attr("id") + " > div.row_val > span",
                        outfit
                    ).text(num);
                    $(
                        "div#" + row.attr("id") + " .tooltip .numAvailable",
                        outfit
                    ).text(have - num);
                }
                if (num === 0) {
    ...
    updateBagSpace: function (currentBagCapacity) {
        ...
        $("#bagspace").text(
            _(
                "free {0}/{1}",
                Math.floor(Path.getCapacity() - currentBagCapacity),
                Path.getCapacity()
            )
        );
    ...
    createOutfittingRow: function (key, num, store) {
        ...
        $("<div>")
            .addClass("upBtn")
            .appendTo(val)
            .click([1], Path.increaseSupply);
        $("<div>")
            .addClass("dnBtn")
            .appendTo(val)
            .click([1], Path.decreaseSupply);
        $("<div>")
            .addClass("upManyBtn")
            .appendTo(val)
            .click([10], Path.increaseSupply);
        $("<div>")
            .addClass("dnManyBtn")
            .appendTo(val)
            .click([10], Path.decreaseSupply);
    ...
    increaseSupply: function (btn) {
        ...
        if (
            ...
            var maxExtraByWeight = Math.floor(
                Path.getFreeSpace() / Path.getWeight(supply)
            );
            var maxExtraByStore =
                $SM.get('stores["' + supply + '"]', true) - cur;
            Path.outfit[supply] =
                cur + Math.min(btn.data, maxExtraByWeight, maxExtraByStore);
